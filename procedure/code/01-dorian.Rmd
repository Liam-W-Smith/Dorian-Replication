...
## Load the original search results. I STARTED HERE! I RAN SOME CODE FOR EACH INDIVIDUAL QUERY, DID NOT UNION THEM OR LOOK CLOSELY ON THE METHODOLOGY ONLINE...
...
```{r}
tevent_rawfull <- dplyr::union(tevent_rawfull, tevent_raw3)
```

```{r}
rm(tevent_rawfull2, tevent_raw, tevent_raw2, tevent_raw2, tevent_raw3, tevent_raw4)
```
...
# Network Analysis THIS LOOKS WEIRD MAYBE FIX IT AFTER LEARNING ABOUT NETWORKS IN DATA SCIENCE
...
## Analyze and graph word assoctiation MOVING ON WILL RUN THIS LATER
```{r word-association}
...
# NEW ADDITIONS
subset data by time or location and see how that affects text analysis or network analysis

maybe do some sentiment analysis, could see how that changes with time

use whatever I learn in data science tomorrow to help me
