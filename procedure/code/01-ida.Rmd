...
```{r}
write.table(control_raw$status_id,
  here("data", "raw", "public", "control_raw.txt"),
  append = FALSE, quote = FALSE, row.names = FALSE, col.names = FALSE
)
```
...
```{r word-frequency}
content_analysis2 = tevent_words %>%
...
```{r save graph}
ggsave(
  here("results","figures","content_analysis2.png"),
  plot = content_analysis2,
  width = 8,
  height = 8,
  units = "in"
)
...
```{r}
counties1 <- readRDS(here("data", "derived", "public", "counties1_tweet_counts.RDS"))
counties2 <- readRDS(here("data", "derived", "public", "counties2_tweet_counts.RDS"))
counties3 <- readRDS(here("data", "derived", "public", "counties3_tweet_counts.RDS"))
```
...
G_Score_Ida3.3 = ggplot() +
  geom_sf(data = counties3, aes(fill = sig), color = "white", lwd = 0.1) +
  scale_fill_manual(
    values = c("#8080FF80", "#FFFFFF80", "#FF808080", "#FF000080"),
    labels = c("slightly low", "insignificant", "", "high"),
    aesthetics = "fill"
  ) +
  guides(fill = guide_legend(title = "Hot Spots")) +
...
ggsave(
  here("results","maps","G_Score_Ida3.3.png"),
  plot = G_Score_Ida3.3,
  width = 8,
  height = 5,
  units = "in"
